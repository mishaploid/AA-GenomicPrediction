...
## This will be the input for GFBLUP to generate an empirical null distribution from 1-50000 markers
...
# a function to sample genes randomly up to a specified number of SNPs (n)

sample_genes <- function(data, n, kbrange) {
  out <- data.frame()
  while (nrow(out) < n) {
    ...
    mn <- min(snp_sub$Position) - kbrange
    mx <- max(snp_sub$Position) + kbrange
...
for (i in args[1]:args[2]) {
  ...
  tryCatch({
  print(snp_number[i,])
  foo <- sample_genes(snps, n = snp_number[i,], kbrange = 2500)
